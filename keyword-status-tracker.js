/**
 *
 * Keyword Status Tracker - Slack Integrated Version
 *
 * Monitors keyword status changes in Google Ads accounts and logs them to a Google Sheet.
 * Sends an alert via Slack whenever keyword statuses change.
 *
 * Tracks changes in:
...
// URL of the spreadsheet to log keyword status changes.
// This spreadsheet needs two sheets:
// 1. "Status Log" - where status changes are recorded
// 2. "Keyword Snapshot" - stores the last known status of each keyword
...
// This sheet should have two columns: Account ID (e.g., 123-456-7890) in the first column,
// and the Slack Webhook URL in the second column. The first row is assumed to be a header.
...
var KEYWORD_SNAPSHOT_SHEET_NAME = 'Keyword Snapshot';
 ...
 */
function main() {
  Logger.log("Starting Keyword Status Tracker...");

  var ss = SpreadsheetApp.openByUrl(SPREADSHEET_URL);

  // Prepare the sheets
  var statusLogSheet = getOrCreateSheet(ss, STATUS_LOG_SHEET_NAME, STATUS_LOG_HEADER);
  var snapshotSheet = getOrCreateSheet(ss, KEYWORD_SNAPSHOT_SHEET_NAME, SNAPSHOT_HEADER);

  // Get the previous keyword snapshot
  var previousSnapshot = getKeywordSnapshot(snapshotSheet);
  Logger.log("Loaded %s keywords from previous snapshot.", Object.keys(previousSnapshot).length);

  // Get current keyword statuses
  var currentKeywords = getCurrentKeywordStatuses();
  Logger.log("Found %s keywords in current account.", Object.keys(currentKeywords).length);

  // Compare and find changes
  var changes = detectStatusChanges(previousSnapshot, currentKeywords);
  ...
  if (changes.length > 0) {
    ...
    Logger.log("No keyword status changes detected.");
  }

  // Update the snapshot with current data
  updateKeywordSnapshot(snapshotSheet, currentKeywords);

  Logger.log("Keyword Status Tracker completed.");
}
...
/**
 * Retrieves the previous keyword snapshot from the spreadsheet
 * @param {Sheet} sheet The snapshot sheet
 * @return {Object} A map of keyword IDs to their status data
 */
function getKeywordSnapshot(sheet) {
  var snapshot = {};
...
/**
 * Gets current keyword statuses from the Google Ads account
 * @return {Object} A map of keyword IDs to their current status data
 */
function getCurrentKeywordStatuses() {
  var keywords = {};
  ...
  try {
    ...
    while (results.hasNext()) {
      ...
      var keywordId = row.adGroupCriterion.criterionId.toString();
    ...
    Logger.log("Error querying keywords: %s", e);
  }
 ...
 * @param {Object} previousSnapshot The previous keyword snapshot
 * @param {Object} currentKeywords The current keyword statuses
 * @return {Array} Array of change objects
 */
function detectStatusChanges(previousSnapshot, currentKeywords) {
  var changes = [];
  var accountName = AdsApp.currentAccount().getName();
  var accountId = AdsApp.currentAccount().getCustomerId();
  var timestamp = new Date();

  // Check for changes in existing keywords and new keywords
  for (var keywordId in currentKeywords) {
    var current = currentKeywords[keywordId];
    var previous = previousSnapshot[keywordId];

    if (!previous) {
      // New keyword - only log if this isn't the first run
  ...
  for (var keywordId in previousSnapshot) {
    if (!currentKeywords[keywordId]) {
      var previous = previousSnapshot[keywordId];
      ...
      changes.push({
        ...
        keywordId: keywordId,
        keywordText: previous.keywordText,
...
/**
 * Updates the keyword snapshot sheet with current data
 * @param {Sheet} sheet The snapshot sheet
 * @param {Object} currentKeywords The current keyword statuses
 */
function updateKeywordSnapshot(sheet, currentKeywords) {
  ...
  for (var keywordId in currentKeywords) {
    var keyword = currentKeywords[keywordId];
    rows.push([
      keyword.keywordId,
  ...
  if (rows.length > 0) {
    ...
    Logger.log("Updated keyword snapshot with %s keywords.", rows.length);
  }
}
...
/**
 * Sends a Slack alert with a summary of keyword status changes.
 * @param {Array} changes Array of change objects
 */
function sendSlackAlert(changes) {
  ...
  var message = '*[Google Ads Alert] - Keyword Status Changes Detected*\n\n' +
    'Account: *' + accountName + '* (' + accountId + ')\n\n' +
    '*Summary of Changes:*\n';

  if (summary.enabled > 0) {
    message += '\u2022 Keywords Enabled: ' + summary.enabled + '\n';
  }
  if (summary.paused > 0) {
    message += '\u2022 Keywords Paused: ' + summary.paused + '\n';
  }
